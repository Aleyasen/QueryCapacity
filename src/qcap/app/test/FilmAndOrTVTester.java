...
public class FilmAndOrTVTester extends TransformationTester {
    ...
    public static List<Integer> tv_program_other_queries;
    public static List<Integer> film_other_queries;

    static {
        tv_program_queries = Arrays.asList(43254, 44178, 44757, 45264, 45990, 46448, 46733, 48395, 48494,
                49034, 49119, 49253, 49969, 50321);
        tv_program_other_queries = Arrays.asList(44124, 46079, 46551, 47906, 49189);
        film_queries = Arrays.asList(43891, 44456, 44882, 45755, 47936, 50041);
        film_other_queries = Arrays.asList(43019, 43267, 43580, 44384,
...
//        testSourceSchema("tv_and_or_film_source_schema_7.txt");
        testTargetSchema("tv_and_or_film_target_schema_7.txt");
    }
    ...
    public static void testSourceSchema(String resultFile) {
        ...
        Collection<Query> tv_program_other_queries_obj = Query.findById(tv_program_other_queries);
        Collection<Query> film_other_queries_obj = Query.findById(film_other_queries);
//        Collection<Query> queries = Query.findById(47654);
        ...
        BaseIndex tv_program_other_index = new BaseIndex(Constants.TBL_TV_PROGRAM_W_TYPE);
        tv_program_other_index.setAcceptList(filmTVacceptList);
        BaseIndex film_other_index = new BaseIndex(Constants.TBL_FILM_W_TYPE);
        film_other_index.setAcceptList(filmTVacceptList);

        execBatchQueries(tv_program_queries_obj, tv_program_index, AppConfig.RESULT_DIR + resultFile);
        execBatchQueries(tv_program_other_queries_obj, tv_program_other_index, AppConfig.RESULT_DIR + resultFile);
        execBatchQueries(film_queries_obj, film_index, AppConfig.RESULT_DIR + resultFile);
        execBatchQueries(film_other_queries_obj, film_other_index, AppConfig.RESULT_DIR + resultFile);
...
//        Collection<Query> queries = Query.findById(47654);
        ...
        tv_program_union_tv_and_film_index.setAcceptList(filmTVacceptList);
        BaseIndex film_union_tv_and_film_index = new BaseIndex(Constants.TBL_FILM_UNION_FILM_AND_TV_PROGRAM);
        film_union_tv_and_film_index.setAcceptList(filmTVacceptList);

        BaseIndex other_1_union_tv_and_film_index = new BaseIndex(Constants.TBL_OTHER_UNION_FILM_AND_TV_PROGRAM);
        other_1_union_tv_and_film_index.setAcceptList(filmTVacceptList);
        BaseIndex other_2_union_tv_and_film_index = new BaseIndex(Constants.TBL_OTHER_UNION_FILM_AND_TV_PROGRAM);
        other_2_union_tv_and_film_index.setAcceptList(filmTVacceptList);


        execBatchQueries(tv_program_queries_obj, tv_program_union_tv_and_film_index, AppConfig.RESULT_DIR + resultFile);
        execBatchQueries(tv_program_other_queries_obj, other_1_union_tv_and_film_index, AppConfig.RESULT_DIR + resultFile);
        execBatchQueries(film_queries_obj, film_union_tv_and_film_index, AppConfig.RESULT_DIR + resultFile);
        execBatchQueries(film_other_queries_obj, other_2_union_tv_and_film_index, AppConfig.RESULT_DIR + resultFile);
